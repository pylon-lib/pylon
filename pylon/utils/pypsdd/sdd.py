...
class NormalizedSddNode(SddNode):
    ...
    def generate_sample(self, batch_size, clear_data=True):
        """Compute the MPE instation given weights associated with literals.
        ...
        for node in self.as_positive_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                data = torch.full((batch_size, 1), node.literal, device=torch.cuda.current_device())
                ...
                sampled_branch = Categorical(probs = node.theta.exp()).sample() #Shape: (batch_size)
                assert(sampled_branch.shape[0] == batch_size)

                data = torch.tensor([], device=torch.cuda.current_device())
                for p, s in node.positive_elements:
                    a = torch.cat((p.data, s.data), dim=1).unsqueeze(dim=0)
                    data = torch.cat((data, a), dim=0)

                data = torch.gather(data, 0, sampled_branch.unsqueeze(0).unsqueeze(-1).expand(-1, -1, data.size(-1))).squeeze(dim=0)
    ...
    #    for node in self.as_positive_list(clear_data=clear_data):
    #        if node.is_false():
    #            data = torch.tensor(-float('inf'), device=torch.cuda.current_device())
    #        elif node.is_true():
    #            theta = [litleaves[node.vtree.var - 1][0], litleaves[node.vtree.var - 1][1]]
    #            data = torch.tensor(0, device=torch.cuda.current_device())
    #        elif node.is_literal():
    #            theta = [node.literal < 0, node.literal > 0]
    #            if node.literal > 0:
    #                data = litleaves[node.literal-1][1]
    #            else:
    #                data = litleaves[-node.literal-1][0]
    #        else: # node.is_decomposition
    #            data = torch.stack([p.data + s.data for p, s in node.positive_elements], dim=-1)
    #            theta = data
    #            data = data.logsumexp(dim=-1).clamp(max=-1.1920928955078125e-07)
    #            theta = theta - data.unsqueeze(-1)
    #            #data = torch.logsumexp([p.data + s.data for p,s in node.elements])
    ...
    def generate_tf_ac(self, litleaves, clear_data=False):
        ...
        for node in self.as_list(clear_data=clear_data):
            if node.is_false():
                ...
                #if len(node.positive_elements) > 10:
                #    print(len(node.positive_elements))
                data = torch.stack([p.data + s.data for p, s in node.elements], dim=-1)
                data = data.logsumexp(dim=-1)#.clamp(max=-1.1920928955078125e-07)
            node.data = data
        return data
    ...
    def Shannon_entropy_stable(self, clear_data=False):
        ...
        for node in self.as_positive_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                entropy = torch.tensor(0.0, device=torch.cuda.current_device())
    ...
    def Shannon_entropy(self, clear_data=False):
        """
        Follows the algorithm from Shih and Ermon (2020) for calculating
        the entropy of a selective SPN.
        """
    ...
    def Shannon_ent(self, clear_data=False):
        """
        Follows the algorithm from Shih and Ermon (2020) for calculating
        the entropy of a selective SPN.
        """
    ...
    def pr_inst(self, inst, clear_data=False):
        ...
        for node in self.as_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                if node.literal > 0:
                    data = torch.tensor(inst[node.literal - 1] > 0, device=torch.cuda.current_device())
                else:
                    data = torch.tensor(inst[-node.literal - 1] <  0, device=torch.cuda.current_device())
    ...
    def hard_sampling(self, clear_data=False):
        ...
        for node in self.as_list(clear_data=clear_data):
            ...
            if node.is_false():
                inst = torch.tensor(0, device=torch.cuda.current_device())
